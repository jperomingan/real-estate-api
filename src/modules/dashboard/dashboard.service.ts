...
export async function getDashboardSummary(
    ...
    return {
        ...
        closedWonLeads,
        leadConversionRate:
            totalLeads > 0 ? Number(((closedWonLeads / totalLeads) * 100).toFixed(2)) : 0,
        totalRevenueRecords: revenues.length,
        totalGrossSales,
        totalCommission,
        totalPaymentReceived,
        totalReceivable: totalGrossSales - totalPaymentReceived,
        paidRevenueCount: revenues.filter((item) => item.paymentStatus === "PAID").length,
        unpaidRevenueCount: revenues.filter((item) => item.paymentStatus === "UNPAID")
            .length,
